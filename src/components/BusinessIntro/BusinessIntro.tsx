...
export default function BusinessIntro() {
  ...
  return (
    <div className="content-shell intro" id="agendar">
      ...
      <BookingTrigger className="primary-button"><span aria-hidden="true">◌</span> Agendar horário</BookingTrigger>
      ...
      <section className="social-section" aria-labelledby="social-title">
        ...
        <nav className="social-links" aria-label="Redes e canais da Lucendy Style">
          {socialLinks.map((link) => link.label === "Agendamento" ? <BookingTrigger key={link.label} ariaLabel="Abrir opções de agendamento"><SocialIcon name={link.icon} /><small>{link.label}</small></BookingTrigger> : <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" aria-label={link.label} title={link.label}><SocialIcon name={link.icon} /><small>{link.label}</small></a>)}
        </nav>
      </section>
      ...
      <BookingTrigger className="booking-card"><span><small>PRONTO PARA MUDAR?</small><strong>Reserve seu horário</strong></span><b>→</b></BookingTrigger>
    </div>
  );
}