  ...
  return (
    <section className="section content-shell" id="servicos" aria-labelledby="services-title">
      ...
      <div className="package-list">{servicePackages.map((pack) => {
        const savings = pack.originalPrice && pack.price ? pack.originalPrice - pack.price : null;
        const percentage = savings && pack.originalPrice ? Math.round((savings / pack.originalPrice) * 100) : null;
        return (
          <article className="package-card" key={pack.id}>
            <div className="package-card__top"><h3>{pack.name}</h3></div>
            {pack.originalPrice && pack.price && savings && percentage ? (
              <div className="package-price">
                <span className="package-price__original">De {money(pack.originalPrice)}</span>
                <div className="package-price__current"><strong>{pack.startingAt && <small>A partir de </small>}{money(pack.price)}</strong><span>{percentage}% OFF</span></div>
                <p>Você economiza {money(savings)}</p>
              </div>
            ) : <div className="package-price package-price--pending"><strong>Valor personalizado</strong><p>{pack.discountNote}</p></div>}
      ...
      <div className="service-list">{services.map((service) => (
        <article className="service-card" key={service.id}>
          ...
          <div className="service-action"><strong>{service.startingAt && <small>A partir de </small>}{money(service.price)}</strong><BookingTrigger message={`Olá! Gostaria de agendar o serviço ${service.name} na Lucendy Style.`} ariaLabel={`Agendar ${service.name}`}>Agendar</BookingTrigger></div>
        </article>
      ))}</div>
    </section>
  );
